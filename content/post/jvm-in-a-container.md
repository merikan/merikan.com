...
Older Java versions uses `cgroup v1` and support for `cgroup v2` was introduced in [Java 15](https://bugs.openjdk.org/browse/JDK-8230305). This means that when using an older Java version, still using `cgroup v1`, the computation of the memory of the JVM is not correct and can cause Out Of Memory. I know that there was some disussions about backporting this change but I am no sure if that really happened.
